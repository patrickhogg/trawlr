...
9. [IPC Layer](#ipc-layer)
10. [Preload Bridge](#preload-bridge)
11. [Renderer Process — Vue 3 Frontend](#renderer-process--vue-3-frontend)
12. [Type System](#type-system)
13. [Data Flow](#data-flow)
14. [Configuration & Settings](#configuration--settings)
15. [Build & Packaging](#build--packaging)
...
Extracts and validates SEO meta tags from HTML content using `cheerio`.

### Audited Tags

| Tag | Validation | Default Limits |
|---|---|---|
| `<title>` | Present, non-empty, within length range | 30–60 characters |
| `<meta name="description">` | Present, non-empty, within length range | 120–160 characters |
| `<meta name="keywords">` | Present, non-empty | N/A (presence only) |
...
| `seo-issues` | `trawlr-seo-issues.csv` | Pages with SEO validation failures |
...
├── Tab Navigation         — All Pages | Broken Links | Redirects | SEO Issues | Settings
│
...
├── SeoIssuesView.vue      — Pages failing meta tag validation
...
```typescript
CrawlSettings       — All user-configurable crawl parameters
...
SeoResult            — SEO audit results for a page's meta tags
RedirectHop          — A single hop in a redirect chain
CrawlResults         — Aggregate of pages + links + progress
...
│   │   ├── SeoAuditor.ts             # Meta tag validation with configurable limits
│   │   └── RobotsParser.ts           # robots.txt fetch, parse, enforce
│   └── ipc/
│       └── crawlHandlers.ts          # IPC handlers + CSV generators
├── src/                               # Vue 3 renderer
